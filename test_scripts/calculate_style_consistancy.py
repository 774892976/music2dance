...
parser.add_argument(
    '--motion_cache_dir', type=str, default='[data_root]/sistpp_feature_mirror/motion_feature_20',
    help='Path to cache dictionary for audio features.')
parser.add_argument(
...
def style_consistancy_score(keypoints3d, sample_num=20):
    scores = []
    ...
    for _ in range(sample_num):
        ...
        seq1_speed = np.sqrt(np.sum((seq_1[1:, :, :] - seq_1[:-1, :, :]) ** 2, axis=-1))
        seq2_speed = np.sqrt(np.sum((seq_2[1:, :, :] - seq_2[:-1, :, :]) ** 2, axis=-1))
        seq1_speed_max = np.max(seq1_speed, axis=0)
        seq2_speed_max = np.max(seq2_speed, axis=0)
        dist = np.sum(np.abs(seq1_speed_max - seq2_speed_max))
        scores.append(dist)
...
def main(_):
    ...
    #     consistancy_score = style_consistancy_score(keypoints3d)
