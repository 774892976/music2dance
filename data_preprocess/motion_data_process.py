...
parser.add_argument(
    '--anno_dir', type=str, default='/nvme/fengbin/music2dance/music2dance_data/aist_plusplus_final',
    help='Path to the AIST++ annotation files.')
parser.add_argument(
    '--audio_dir', type=str, default='/nvme/fengbin/music2dance/music2dance_data/wav',
    help='Path to the AIST wav files.')
parser.add_argument(
    '--audio_cache_dir', type=str, default='/nvme/fengbin/music2dance/music2dance_data/aistpp_feature_mirror/audio_feature_20',
    help='Path to cache dictionary for audio features.')
parser.add_argument(
    '--motion_cache_dir', type=str, default='/nvme/fengbin/music2dance/music2dance_data/aistpp_feature_mirror/motion_feature_20',
    help='Path to cache dictionary for audio features.')
parser.add_argument(
...
def write_data(smpl_trans, smpl_poses, contacts, seq_name, seq_len):
    smpl_poses = smpl_poses.as_matrix().reshape(seq_len, -1)
    smpl_motion = np.concatenate([smpl_trans, smpl_poses, contacts], axis=-1)
    # down sample to 20fps
...
def main():
    ...
    for i, seq_name in enumerate(tqdm(seq_names)):
        ...
        keypoint3d = AISTDataset.load_keypoint3d(dataset.keypoint3d_dir, seq_name, use_optim=False)
        ...
        write_data(smpl_trans, smpl_poses, contacts, seq_name, seq_len)

        smpl_trans_mirror, smpl_poses_mirror = mirror_aug(smpl_trans, smpl_poses, seq_len)
        contacts_mirror = np.concatenate([contacts_r, contacts_l], axis=-1)
        write_data(smpl_trans_mirror, smpl_poses_mirror, contacts_mirror, seq_name + "mirror", seq_len)
    ...
    if FLAGS.split == "testval":
        ...
        for i, seq_name in enumerate(tqdm(seq_names)):
            ...
            smpl_trans /= smpl_scaling
            seq_len = smpl_poses.shape[0]
            keypoint3d = AISTDataset.load_keypoint3d(dataset.keypoint3d_dir, seq_name, use_optim=True)
            contacts_l, contacts_r = extract_foot_contact(keypoint3d)
            smpl_poses = R.from_rotvec(
                smpl_poses.reshape(-1, 3))
            contacts = np.concatenate([contacts_l, contacts_r], axis=-1)
